...
BEGIN
	...
	(
		Id int IDENTITY(1, 1) NOT NULL PRIMARY KEY,
		ReferenceId uniqueidentifier NOT NULL CONSTRAINT [DF_RentPayment_ReferenceId] DEFAULT NEWID(),
		...
		CONSTRAINT [FK_RentPayment_RentArrangement] FOREIGN KEY (RentArrangementId)
			REFERENCES dbo.RentArrangement (Id),
		CONSTRAINT [FK_RentPayment_WeeklyCalendar] FOREIGN KEY (WeeklyCalendarId)
			REFERENCES dbo.WeeklyCalendar (Id)
	)
END
GO
...
INSERT INTO RentPayment (RentArrangementId, WeeklyCalendarId, DueAmount, DueDate)
SELECT ra.Id, wc.CalendarId, ra.WeeklyRent, wc.WeekEnd
FROM RentArrangement ra
CROSS APPLY 
	(
		SELECT 
			Id AS CalendarId,
		...
		WHERE wc.StartDate > ra.StartDate AND wc.EndDate <= ra.EndDate
	) wc;